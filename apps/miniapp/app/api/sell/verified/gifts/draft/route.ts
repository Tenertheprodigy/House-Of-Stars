...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!state?.order_id)
    return NextResponse.json(
      { error: "Complete the wallet step first." },
      { status: 404 },
    );
  const [{ data: order }, { data: evidence }] = await Promise.all([
    supabase
      .from("orders")
      .select(
        "id, order_number, stars_amount, payout_asset, payout_network, wallet_address, expected_payout_amount, status, settlement_available_at",
      )
  ...
  return order
    ? NextResponse.json({ order, evidence: evidence ?? [] })
    : NextResponse.json({ error: "Draft unavailable." }, { status: 404 });
}
